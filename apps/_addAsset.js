...
businessNetworkConnection.connect(connectionProfile, businessNetworkIdentifier, participantId, participantPwd)
    .then((result) => {
        ...
        return businessNetworkConnection.getAssetRegistry('org.acme.biznet.PurchaseOrderAsset');
    }).then((result) => {
...
asset PurchaseOrderAsset identified by orderId {
  ...
  o PurchaseOrderAsset[] subPurchaseOrders
        ...
        */
        ...
        let compositeOrder = factory.newResource('org.acme.biznet', 'PurchaseOrderAsset', 'comp:142',{generate:'sample'});
        compositeOrder.assetId='asset42';
        console.log(compositeOrder);
        compositeOrder.subPurchaseOrders = null;
        return assetRegistry.add(compositeOrder);
        // people.forEach((name) => {
        //     for (let i = 1; i < 2; i++) {
        //         let id = name + ':' + faker.random.number(100000);
        //         let asset = factory.newResource('org.acme.biznet', 'PurchaseOrderAsset', id);
        //         asset.assetId = i + ' Conga Coin';
        //         asset.subPurchaseOrders = [];
        //         assets.push(asset);
        //     }
        // });
