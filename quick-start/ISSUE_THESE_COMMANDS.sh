...
export HL_COMPOSER_CLI=$(which composer)
...
if [ -z "${BNA_FILE}" ]; then
  BNA_FILE=../networks/violet-blue-green/dist/vbg-network.bna
fi
...
composer network start  --card PeerAdmin@hlfv1  -A admin -S adminpw  --file ./cards/admin.card --networkName ${NETWORK_NAME} --networkVersion ${NETWORK_VERSION}
composer card import --file ./cards/admin.card --card admin@bsn-local
